...
# Gateway port: 18789
# Auth token: from hooks.token in config (separate from gateway.auth.token)
```
...
TOKEN="<your-hooks-token>"  # from config hooks.token
...
# Dispatch with wake callback (use hooks.token from config)
...
# Dispatch fixes with wake callbacks (use hooks.token from config)
...
- Verify the IP/token are correct (use hooks.token, not gateway.auth.token)
